 ...
 */
@RestController
@RequestMapping("/auth/api/recordCouponThaw")
public class AuthApiRecordCouponThawController {
    @Autowired
    private IRecordCouponThawService iRecordCouponThawService;

    @PostMapping("/getAll")
    @ResponseBody
    public ApiResult<MyPageInfo<RecordVo>> getAll(@RequestBody @Validated RecordDto recordCouponDto){
        return ApiResult.success(iRecordCouponThawService.getAll(recordCouponDto));
    }
}
